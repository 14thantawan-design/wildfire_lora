...
ในหัวแผนที่จะมีปุ่ม `ค้นหา GPS ใหม่` สำหรับจุดตรวจที่เลือก คลิกวงกลมของโหนดบนแผนที่ก่อน แล้วกดปุ่ม ระบบจะเก็บคำสั่งไว้และส่งผ่าน Gateway เมื่อโหนดส่งข้อมูลรอบถัดไป หลังจากนั้นโหนดจะล้างพิกัดเดิมและเริ่มค้นหา GPS ใหม่
...
ฟังก์ชันนี้ต้องอัปโหลดโค้ดเวอร์ชันปัจจุบันลงทั้ง Gateway และ Sensor Node จึงจะทำงานครบเส้นทาง หากหน้าเว็บอยู่ในโหมดข้อมูลทดลอง ปุ่มจะถูกปิดไว้
